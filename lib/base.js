	...
	class Daily {
		...
		setDate(unix) {
			let date = this.getDate(),
				moment = window.Daily.Dictionary.daily()
					+ String.fromCharCode(32)
					+ window.moment.unix(unix).utc().format(this.constructor.format()),
		...
		getWrapper() {
			if (this.elements.hasOwnProperty('div')) return this.elements.div;
			let figure = this.getFigure(),
				date = this.getDate();

			this.elements.div = document.createElement('div');
			this.elements.div.className = 'daily';
			this.elements.div.appendChild(date);
			this.elements.div.appendChild(figure);
